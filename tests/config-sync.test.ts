...
const BIN = fileURLToPath(new URL("../src/config-sync.ts", import.meta.url));
 ...
 */
function harnessManifest(liveRoot: string, extraFiles: Record<string, string> = {}): string {
...
function makeFixture(
	...
	writeFileSync(harnessPath, harnessManifest(liveRoot, opts.extraManifestFiles));
...
describe("config-sync (four branches per file, AC #2)", () => {
	test("tracked edited (tracked!=manifest, live==manifest): copy tracked -> live", () => {
		const f = makeFixture({
			...
			liveFiles: { "AGENTS.md": THIN, "agents/a.md": "agent a\n", "opencode.jsonc": "{}\n" },
		});
		...
		const report = JSON.parse(r.stdout);
		assert.equal(report.files["AGENTS.md"].action, "copy");
		assert.equal(readFileSync(join(f.liveRoot, "AGENTS.md"), "utf8"), `${THIN}edited\n`);
		assert.equal(stateFiles(f.statePath)["AGENTS.md"], sha256(`${THIN}edited\n`));
	});
	...
	test("live edited (live!=manifest, tracked==manifest): refuse, name file + both fixes", () => {
		...
		const refusal = report.refusals.find((x: { file: string }) => x.file === "AGENTS.md");
		assert.ok(refusal);
		assert.ok(refusal.fix.includes("--adopt"));
		assert.ok(refusal.fix.includes("--force"));
		// nothing mutated
		assert.equal(readFileSync(join(f.liveRoot, "AGENTS.md"), "utf8"), `${THIN}local-edit\n`);
	...
	test("live edited + --adopt: copy live into repo, update manifest", () => {
		...
		assert.equal(readFileSync(join(f.trackedRoot, "AGENTS.md"), "utf8"), localEdit);
		// live unchanged, manifest now records the adopted live bytes
		assert.equal(readFileSync(join(f.liveRoot, "AGENTS.md"), "utf8"), localEdit);
	...
	test("both differ: conflict, refuse always, no flag rescues", () => {
		...
		for (const extra of [[], ["--force"], ["--adopt"], ["--force", "--adopt"]]) {
		...
		assert.equal(readFileSync(join(f.trackedRoot, "AGENTS.md"), "utf8"), `${THIN}tracked-change\n`);
		assert.equal(readFileSync(join(f.liveRoot, "AGENTS.md"), "utf8"), `${THIN}live-change\n`);
	});
...
describe("config-sync (never deletes, exclusions, untracked live files)", () => {
	test("untracked live files (incl. secrets/ and node_modules/) survive a sync untouched", () => {
		...
		writeFileSync(join(f.liveRoot, "node_modules", "pkg", "index.js"), "module\n");
		const r = syncRun(f);
		assert.equal(r.status, 0);
		assert.equal(readFileSync(join(f.liveRoot, "untracked.txt"), "utf8"), "keep me\n");
		assert.equal(readFileSync(join(f.liveRoot, "secrets", "auth.json"), "utf8"), "secret\n");
		assert.equal(
	...
	test("manifest entry under an excluded prefix is refused, nothing mutated", () => {
		...
		assert.equal(r.status, 1);
		assert.equal(
			JSON.parse(r.stdout).refusals.some((x: { file: string }) => x.file === "secrets/leak.json"),
			true,
		);
		assert.equal(readFileSync(join(f.liveRoot, "secrets", "leak.json"), "utf8"), "nope\n");
	});
});
...
describe("config-sync (crash recovery, AC review finding 1)", () => {
	test("partial-failure (later copy throws): state stays stale, re-run converges to skip, not conflict", {
		...
		writeFileSync(join(f.trackedRoot, "config", "agents", "a.md"), "agent a edited\n");
		...
		assert.equal(readFileSync(join(f.liveRoot, "AGENTS.md"), "utf8"), trackedEdit);
		...
		assert.equal(stateFiles(f.statePath)["agents/a.md"], sha256("agent a edited\n"));
...
describe("config-sync (path normalization, AC review finding 2)", () => {
	...
	test("agents/../secrets/x normalizes to secrets/x and is excluded", () => {
		const f = makeFixture({
			...
			liveFiles: { "AGENTS.md": THIN, "agents/a.md": "agent a\n", "opencode.jsonc": "{}\n" },
			extraManifestFiles: { "agents/../secrets/x.json": "agents/../secrets/x.json" },
		});
		mkdirSync(join(f.liveRoot, "secrets"), { recursive: true });
		...
		const report = JSON.parse(r.stdout);
		assert.equal(
			report.refusals.some((x: { file: string }) => x.file === "secrets/x.json"),
			true,
		);
		assert.equal(readFileSync(join(f.liveRoot, "secrets", "x.json"), "utf8"), "nope\n");
	});
...
describe("config-sync (flag path expansion, AC review finding 3)", () => {
	test("~/... flag values expand via HOME (--harness-manifest, --live-root, --state)", () => {
		...
		writeFileSync(join(fakeHome, "harness.json"), harnessManifest(join(fakeHome, "live")));
		const r = run(
			[
				"--tracked-root",
				trackedRoot,
		...
		assert.equal(report.state, join(fakeHome, "state", "config-materialize.json"));
		assert.equal(existsSync(join(fakeHome, "live", "AGENTS.md")), true);
		assert.equal(existsSync(join(fakeHome, "live", "opencode.jsonc")), true);
		assert.equal(existsSync(join(fakeHome, "state", "config-materialize.json")), true);
	});
});
...
describe("config-sync (symlink safety, AC review finding 4)", () => {
	test("symlinked live file: exit 2 refusal, nothing written through the link", () => {
		...
		assert.equal(lstatSync(join(f.liveRoot, "AGENTS.md")).isSymbolicLink(), true);
	});
...
describe("config-sync (default trackedRoot resolution, trackedRoot '.')", () => {
	test("default trackedRoot resolves '.' against the repo root; AGENTS.md at the repo root is found", () => {
		...
		const r = run(["--harness-manifest", harness, "--live-root", liveRoot, "--state", statePath]);
		assert.equal(r.status, 0);
		const report = JSON.parse(r.stdout);
