...
import { headroomCompress } from "../src/headroom-compress.ts";

const BIN = fileURLToPath(new URL("../src/headroom-compress.ts", import.meta.url));
...
describe("headroom-compress (happy paths, live proxy)", () => {
	test("compresses a 2K-token payload with tokens_saved > 0", { skip: SKIP_LIVE }, async () => {
	...
	test('mode "ccr" populates ccr_hashes', { skip: SKIP_LIVE, timeout: 60_000 }, async () => {
...
describe("headroom-compress (reviewer fix round)", () => {
	function stubServer(body: unknown, capture?: { request?: unknown }): Promise<string> {
