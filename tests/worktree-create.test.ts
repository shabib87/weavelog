...
const BIN = fileURLToPath(
  new URL("../src/tools/worktree-create.ts", import.meta.url),
);
const TASK_VALIDATE_SRC = fileURLToPath(
...
function setList(tasks: object[]) {
  writeFileSync(
    listFile,
    JSON.stringify({ schemaVersion: 1, kind: "task-list", tasks }),
  );
}
...
describe("worktree-create (happy paths)", () => {
  ...
  test("--dry-run makes no changes but exits 0", () => {
    ...
    assert.equal(
      existsSync(join(repoDir, ".worktrees", SAMPLE_TASK.id)),
      false,
    );
    assert.equal(
      existsSync(join(repoDir, ".git", "hooks", "pre-commit")),
      false,
    );
  });
...
describe("worktree-create (review fixes)", () => {
  ...
  test("checked AC renders - [x]", () => {
    ...
    const content = readFileSync(
      join(repoDir, ".worktrees", SAMPLE_TASK.id, "TASK.md"),
      "utf8",
    );
...
`;
  ...
  const TaskPath = "backlog/tasks/task-9 - Do thing.md";
  ...
  function setupRepoWithTaskFile(acCount: number, label = "spec-approved") {
    ...
    writeFileSync(join(repoDir, TaskPath), taskFile(acCount, label));
    spawnSync("git", ["add", TaskPath], { cwd: repoDir, encoding: "utf8" });
  ...
  function runHook(opts: { withWeavelog?: boolean } = {}) {
    const path =
      opts.withWeavelog === false
        ? process.env.PATH
        : `${fakeWeavelogShim()}:${process.env.PATH}`;
  ...
  test("end-to-end: hook passes when a non-spec-approved task's ACs are gutted", () => {
    ...
    writeFileSync(join(repoDir, TaskPath), taskFile(1, "deferred"));
    spawnSync("git", ["add", TaskPath], { cwd: repoDir, encoding: "utf8" });
    const ok = runHook();
    assert.equal(ok.status, 0);
  });
