...
import {
	...
	runProxyChecks,
	stripJsoncComments,
} from "../src/stack-check.ts";

const BIN = fileURLToPath(new URL("../src/stack-check.ts", import.meta.url));
...
srv.listen(0,"127.0.0.1",()=>{console.log("PORT:"+srv.address().port)})`,
	...
	const port = await new Promise<string>((resolve, reject) => {
		const timeout = setTimeout(() => reject(new Error("stub server did not report port")), 10_000);
	...
	MODELS_URL = `http://127.0.0.1:${port}/api/v1/models`;
	NPM_URL = `http://127.0.0.1:${port}/backlog.md/latest`;
});
after(() => {
	stubProc?.kill();
});
...
function makeStackHome(): string {
	const home = join(tmpdir(), `stack-home-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	writeFileSync(join(home, ".local", "bin", "markitdown"), '#!/bin/sh\necho "markitdown 0.1.7"\n');
...
describe("stack-check (happy paths)", () => {
	...
	test("report no longer checks superpowers (plugin removed 2026-08-16)", () => {
		...
		assert.ok(!JSON.stringify(report.checks.opencode ?? {}).includes("superpowersPlugin"));
		rmSync(home, { recursive: true, force: true });
	});
...
describe("checkDiagramDesign (drift detection)", () => {
	...
	test("drift when installed HEAD differs from pinned", () => {
		...
		assert.ok(drift.includes(`diagram-design skill ${full} != pinned deadbeef`));
		assert.equal(check.installed, true);
		rmSync(dir, { recursive: true, force: true });
	});

	test("drift when skill directory is missing", () => {
		const missing = join(tmpdir(), `stack-check-dd-missing-${Date.now()}`, "diagram-design");
	...
	test("drift when skill directory lacks git metadata", () => {
		...
		assert.ok(
			drift.includes(`diagram-design git metadata unavailable at ${empty} (pinned ac490fd)`),
		);
		assert.equal(check.installed, true);
...
describe("checkDiagramDesign (main() wiring)", () => {
	test("spawned run reads skills via STACK_CHECK_SKILLS_DIR", () => {
		...
		const { r, home } = stackRun(
			["--no-notify", "--json", join(tmpdir(), `stack-check-dd-int-${Date.now()}.json`)],
...
describe("checkMarkitdown (drift detection)", () => {
	...
	test("version prefix is stripped and drift names manifest -> installed", () => {
		...
		try {
			const { check, drift } = checkMarkitdown(fakeMarkitdown(dir, "0.2.0"), "0.1.7");
	...
	test("missing binary is drift and reports (missing)", () => {
		const missing = join(tmpdir(), `stack-check-mi-missing-${Date.now()}`, "markitdown");
		const { check, drift } = checkMarkitdown(missing, "0.1.7");
		assert.equal(check.current, "(missing)");
		assert.ok(drift.includes(`markitdown binary missing at ${missing} (manifest 0.1.7)`));
	});
	...
	test("unset manifest key reports an explicit missing-key drift (not a bogus -> install)", () => {
		...
		try {
			...
			assert.ok(drift.includes("markitdown manifest key missing (installed 0.1.7)"));
		} finally {
			rmSync(dir, { recursive: true, force: true });
		}
	});
...
function makeStubFetch(opts: StubFetchOpts): ProxyFetch {
	return async (url: string) => {
		if (url.endsWith("/health")) {
			if (opts.healthThrow) throw new Error("network failure: connection refused");
			return {
		...
		if (url.endsWith("/stats")) {
			if (opts.statsThrow) throw new Error("network failure: connection refused");
			return {
...
function proxyFixture(opts: FixtureOpts) {
	...
	if (opts.modelsJson !== undefined) writeFileSync(piModelsPath, opts.modelsJson);
	if (opts.opencodeJsonc !== undefined) writeFileSync(opencodeConfigPath, opts.opencodeJsonc);
	if (opts.extDir) mkdirSync(extDir, { recursive: true });
	if (opts.settingsJson !== undefined) writeFileSync(settingsPath, opts.settingsJson);
	return {
		dir,
		piModelsPath,
		opencodeConfigPath,
...
describe("stack-check proxy doctor (pure checks)", () => {
	test("checkProxyHealth passes when ready:true", () => {
		const { check, drift } = checkProxyHealth({ status: "healthy", ready: true }, PROXY_HOST);
		assert.deepEqual(drift, []);
		assert.equal(check.healthy, true);
	});

	test("checkProxyHealth fails when ready:false", () => {
		const { check, drift } = checkProxyHealth({ status: "unhealthy", ready: false }, PROXY_HOST);
	...
	test("checkProxyMode passes when summary.mode === cache", () => {
		const { check, drift } = checkProxyMode({ summary: { mode: "cache" } }, "cache", PROXY_HOST);
		assert.deepEqual(drift, []);
		assert.equal(check.mode, "cache");
	});

	test("checkProxyMode fails on wrong mode (token)", () => {
		const { check, drift } = checkProxyMode({ summary: { mode: "token" } }, "cache", PROXY_HOST);
		assert.equal(drift.length, 1);
	...
	test("checkPiModelsConfig fails on missing baseUrl", () => {
		const { drift } = checkPiModelsConfig({ providers: { openrouter: {} } }, PI_BASE);
		assert.equal(drift.length, 1);
	...
	test("checkThresholds fails when both missing (empty object)", () => {
		...
		assert.equal(
			drift.some((d) => d.includes("minContextTokens") && d.includes("missing")),
			true,
		);
		assert.equal(
...
describe("stack-check proxy doctor (runProxyChecks orchestration)", () => {
	...
	test("JSON parse error on pi models.json produces a drift", async () => {
		...
		try {
			...
			assert.equal(
				drift.some((d) => d.includes("cannot parse") && d.includes("models.json")),
				true,
			);
		} finally {
			f.cleanup();
  ...
  "provider": {
	...
	test("network failure on /health records checks.proxyHealth with reachable:false", async () => {
		...
		try {
			...
			assert.equal((checks.proxyHealth as Record<string, unknown>).reachable, false);
		} finally {
			f.cleanup();
		}
	});
	...
	test("network failure on /stats records checks.proxyMode with reachable:false", async () => {
		...
		try {
			...
			assert.equal((checks.proxyMode as Record<string, unknown>).reachable, false);
		} finally {
			f.cleanup();
		}
	});
...
describe("stack-check proxy doctor (CLI --proxy)", () => {
	const proxyEnv = (f: ReturnType<typeof proxyFixture>): Record<string, string> => ({
		HOME: makeStackHome(),
	...
	test("--proxy with all-green env-overridden configs writes a structured report", () => {
		...
		const reportPath = join(tmpdir(), `stack-check-proxy-cli-${Date.now()}.json`);
		try {
			const r = run(["--proxy", "--no-notify", "--json", reportPath], proxyEnv(f));
	...
	test("--proxy with broken pi config exits 1 and names the fix", () => {
		...
		const reportPath = join(tmpdir(), `stack-check-proxy-bad-${Date.now()}.json`);
		try {
			const r = run(["--proxy", "--no-notify", "--json", reportPath], proxyEnv(f));
...
function makeConfigFixture(opts: {
	...
	for (const rel of [...Object.keys(opts.tracked ?? {}), ...Object.keys(opts.live ?? {})]) {
	...
	const manifestPath = join(trackedRoot, "config", "harnesses", "opencode.json");
...
describe("checkConfigDrift (config/ vs live per harness manifest)", () => {
	...
	test("manifest rel escaping the roots: drift entry naming the escape, legit entries unaffected", () => {
		...
		try {
			...
			assert.equal(
				drift.some((d) => d.includes("AGENTS.md") && d.includes("config drift")),
				false,
			);
		} finally {
			f.cleanup();
...
describe("checkPointerTargets (thin global AGENTS.md pointer validation, AC #10)", () => {
	...
	test("ignores unquoted paths and backtick-quoted relative paths", () => {
		...
		mkdirSync(home, { recursive: true });
		try {
			const content = "read ~/unquoted and `relative/path` but not ~/quoted-either";
			const { drift, check } = checkPointerTargets(content, "/fake/AGENTS.md", home);
			assert.deepEqual(drift, []);
...
describe("stack-check config drift (CLI wiring, env overrides)", () => {
	test("drifted config file appears in the report with both fixes", () => {
		...
		const reportPath = join(tmpdir(), `stack-check-config-cli-${Date.now()}.json`);
		try {
	...
	test("clean env-overridden config: no config drift lines in the report", () => {
		...
		const reportPath = join(tmpdir(), `stack-check-config-clean-${Date.now()}.json`);
		try {
			const { r, home } = stackRun(["--no-notify", "--json", reportPath], {
				STACK_CHECK_CONFIG_DIR: f.trackedRoot,
				STACK_CHECK_CONFIG_LIVE_DIR: f.liveRoot,
