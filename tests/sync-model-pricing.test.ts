...
const BIN = fileURLToPath(new URL("../src/sync-model-pricing.ts", import.meta.url));
...
let MODELS_URL = "http://127.0.0.1:0/api/v1/models";
...
before(async () => {
	...
	MODELS_URL = await new Promise((resolve, reject) => {
...
function run(args: string[], extraEnv: Record<string, string> = {}) {
	return spawnSync(process.execPath, ["--import", "tsx", BIN, ...args], {
		encoding: "utf8",
		env: { ...process.env, OPENROUTER_MODELS_URL: MODELS_URL, ...extraEnv },
		timeout: 120_000,
	});
}
...
function makeFakeHome(dir: string, models: string[]): string {
	const home = join(dir, `home-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe("sync-model-pricing (happy paths)", () => {
	...
	test("--apply injects entries with cache pricing and is idempotent", () => {
		...
		assert.equal(db2["openrouter/z-ai/glm-5.3-flash"].input_cost_per_token, e.input_cost_per_token);
