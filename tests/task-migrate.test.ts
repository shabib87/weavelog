...
import { planMigration, type TaskPlan } from "../src/task-migrate.ts";

const BIN = fileURLToPath(new URL("../src/task-migrate.ts", import.meta.url));
...
function taskMd(id: string, status: string, labels: string[], milestone?: string): string {
	const labelBlock = labels.length ? labels.map((l) => `  - ${l}`).join("\n") : "  []";
...
beforeEach(() => {
	...
	writeFileSync(join(tasksDir, "task-3 - immediate.md"), taskMd("TASK-3", "To Do", ["immediate"]));
	writeFileSync(join(tasksDir, "task-4 - bugfix.md"), taskMd("TASK-4", "To Do", ["bugfix"]));
	writeFileSync(
	...
	writeFileSync(join(tasksDir, "task-9 - empty.md"), taskMd("TASK-9", "To Do", []));
	...
	writeFileSync(join(tasksDir, "task-10 - done-v1.md"), taskMd("TASK-10", "Done", ["v1", "infra"]));
	writeFileSync(join(tasksDir, "task-11 - done-empty.md"), taskMd("TASK-11", "Done", []));
});
...
function run(args: string[], extraEnv: Record<string, string> = {}) {
	return spawnSync(process.execPath, ["--import", "tsx", BIN, ...args], {
		encoding: "utf8",
		env: { ...process.env, TASK_MIGRATE_BACKLOG: fakeBin, TASK_MIGRATE_DIR: root, ...extraEnv },
		timeout: 30_000,
	});
}
