...
const BIN = fileURLToPath(new URL("../src/prefix-diff.ts", import.meta.url));
...
describe("prefix-diff (happy paths)", () => {
	...
	test("TOKEN_MODE_HISTORY_REWRITE (OpenAI handler): null compressed_messages + content transforms + zero cache", () => {
		...
		const entry1 = makeEntry({
			request_messages: [sys, user1],
			transforms_applied: ["router:text:0.86", "openai:chat:tool_schema_compaction"],
			cache_read_tokens: 0,
		});
		const entry2 = makeEntry({
			request_messages: [sys, user1, assistant1, user2],
			transforms_applied: ["router:text:0.84", "openai:chat:tool_schema_compaction"],
			cache_read_tokens: 0,
		});
	...
	test("TOKEN_MODE_HISTORY_REWRITE (real transforms): router:mixed + router:config with zero cache", () => {
		...
		const entry1 = makeEntry({
			request_messages: [sys, user1],
			transforms_applied: ["router:mixed:0.82", "openai:chat:tool_schema_compaction"],
			cache_read_tokens: 0,
		});
		const entry2 = makeEntry({
			request_messages: [sys, user1, assistant1, user2],
			transforms_applied: ["router:config:0.36", "openai:chat:tool_schema_compaction"],
			cache_read_tokens: 0,
		});
	...
	test("OPENCODE_DYNAMIC_PREFIX: system message changed in request_messages", () => {
		...
		const entry1 = makeEntry({
			request_messages: [sys_v1, user1],
			compressed_messages: [sys_v1, user1],
		});
		const entry2 = makeEntry({
			request_messages: [sys_v2, user1, user2],
			compressed_messages: [sys_v2, user1, user2],
		});
