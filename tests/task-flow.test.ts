...
const BIN = fileURLToPath(new URL("../src/task-flow.ts", import.meta.url));
...
beforeEach(() => {
	dir = join(tmpdir(), `task-flow-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
function writeTaskState(task: Record<string, unknown>) {
	writeFileSync(stateFile, JSON.stringify({ schemaVersion: 1, kind: "task-view", task }));
}
...
describe("task-flow (happy paths)", () => {
	...
	test("note appends via --append-notes", () => {
		...
		assert.equal(r.status, 0);
		assert.ok(readFileSync(callsLog, "utf8").includes("task edit task-1 --append-notes"));
	});
...
describe("task-flow claim gate (TASK-51 spec-quality gate)", () => {
	...
	test("claim on a regular task with an EARS-violating AC -> exit 1", () => {
		writeTaskState({
			...VALID_TASK,
			acceptanceCriteria: [{ index: 1, text: "The system must be fast", checked: false }],
		});
		const r = run(["claim", "task-1"]);
		assert.equal(r.status, 1);
	...
	test("claim on a chore task with an EARS-violating AC -> warns but proceeds", () => {
		writeTaskState({
			...VALID_TASK,
			type: "chore",
			acceptanceCriteria: [{ index: 1, text: "Reorganize the docs folder", checked: false }],
		});
		const r = run(["claim", "task-1"]);
		assert.equal(r.status, 0);
