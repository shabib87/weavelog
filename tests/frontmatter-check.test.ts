...
const BIN = fileURLToPath(new URL("../src/frontmatter-check.ts", import.meta.url));
...
`,
	...
	test("invalid status enum value exits 1", () => {
		const dir = makeDocs({
			"2026-08-16-badstatus.md": VALID_DOC.replace("status: open", "status: banana"),
		});
		const r = run([dir]);
		assert.equal(r.status, 1);
...
describe("frontmatter-check (reviewer fix round)", () => {
	...
	test("--dry-run reports post-fix validity without writing the file", () => {
		...
		assert.equal(r.status, 0);
		assert.equal(readFileSync(join(dir, "2026-08-16-fixme.md"), "utf8"), before);
	});
	...
	test("superseded check is per-directory: a child in another dir does not satisfy it", () => {
		...
		assert.equal(r.status, 1);
		const report = JSON.parse(r.stdout);
		const parent = report.files.find((f: { file: string }) => f.file.includes("parent"));
...
`;
	...
	test("unknown --schema value exits 2", () => {
		const dir = makeDocs({ "2026-08-20-a.md": ARCH_VALID_A });
	...
	test("valid arch doc with reciprocal related_to exits 0 and no warnings", () => {
		const dir = makeDocs({
			"2026-08-20-a.md": ARCH_VALID_A,
			"2026-08-20-b.md": ARCH_VALID_B_RECIP,
		});
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 0);
		const report = JSON.parse(r.stdout);
		assert.equal(report.summary.invalid, 0);
		const a = report.files.find((f: { file: string }) => f.file.includes("2026-08-20-a.md"));
	...
	test("valid arch doc with non-reciprocal related_to exits 0 BUT warnings non-empty", () => {
		const dir = makeDocs({
			"2026-08-20-a.md": ARCH_VALID_A,
			"2026-08-20-b.md": ARCH_VALID_B_NO_RECIP,
		});
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 0);
		const report = JSON.parse(r.stdout);
	...
	test("status not in architecture enum exits 1", () => {
		const dir = makeDocs({
			"2026-08-20-a.md": ARCH_VALID_A.replace("status: draft", "status: open"),
			"2026-08-20-b.md": ARCH_VALID_B_RECIP,
		});
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 1);
	...
	test("type not in {architecture, adr} exits 1", () => {
		const dir = makeDocs({
			"2026-08-20-a.md": ARCH_VALID_A.replace("type: architecture", "type: recipe"),
			"2026-08-20-b.md": ARCH_VALID_B_RECIP,
		});
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 1);
	...
	test("empty related_to [] is valid (no dangling, no reciprocity check)", () => {
		const dir = makeDocs({ "2026-08-20-solo.md": ARCH_VALID_B_NO_RECIP });
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 0);
		const report = JSON.parse(r.stdout);
		assert.equal(report.files[0].ok, true);
	...
	test("date not matching filename prefix exits 1", () => {
		const dir = makeDocs({
			"2026-08-11-mismatch.md": ARCH_VALID_A,
			"2026-08-20-b.md": ARCH_VALID_B_RECIP,
		});
		const r = run(["--schema", "architecture", dir]);
		assert.equal(r.status, 1);
