...
test("every payload skill declares license and upstream", () => {
	...
	for (const skill of skills) {
		const content = readFileSync(new URL(`${skill}/SKILL.md`, SKILLS_ROOT), "utf8");
		assert.ok(/^license: /m.test(content), `${skill}/SKILL.md missing license line`);
		assert.ok(/^upstream:/m.test(content), `${skill}/SKILL.md missing upstream line`);
	}
});
