...
Escalation ladder (ADR-004): your rung is fixed by your agent file's model field — L1 deepseek-v4-pro-0813, L2 glm-5.3, L3 qwen3.8-2.4t-a95b, L4 kimi-k3. Plan review starts at L2 (architecture-scoped by definition); L1 is for same-family rechecks of deepseek-authored plans; →L3 on two consecutive rework-cycle failures or harness/orchestration plans; →L4 on L3 disagreement or image/screenshot input required. A plan never silently skips a level. The `weavelog check` risk-signals gate (TASK-78) computes the risk-signal triggers deterministically; the conductor applies them at dispatch.
