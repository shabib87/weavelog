...
const BACKLOG =
  process.env.WORKTREE_BACKLOG ?? join(homedir(), ".bun", "bin", "backlog");
...
function resolveRepoRoot(): string {
  ...
  if (r.status !== 0)
    fail2(`git rev-parse --show-toplevel failed: ${r.stderr || r.stdout}`);
...
function resolveHookPath(repoRoot: string): string {
  ...
  if (r.status !== 0)
    fail2(`git rev-parse --git-path failed: ${r.stderr || r.stdout}`);
...
function handleHook(repoRoot: string, dryRun: boolean): string {
  ...
  if (hookExists) {
    ...
    if (!isOurs)
      return "hook=FOREIGN — main-block NOT installed, reconcile manually";
...
function fetchTaskDetails(taskId: string): TaskInfo {
  ...
  if (!vr.ok)
    fail2(`backlog task view failed for ${taskId}: ${vr.err || vr.out}`);
  try {
...
function processTaskId(
  ...
  if (dryRun) {
    console.log(
      `DRY-RUN ${taskId} ${wtPath} branch=${branch} task.md=would-write ${hookStatus}`,
    );
    return;
  }
  ...
  if (wtAdd.status !== 0)
    fail2(
      `git worktree add failed for ${taskId}: ${wtAdd.stderr || wtAdd.stdout}`,
    );

  writeFileSync(join(wtPath, "TASK.md"), formatTaskMd(task));
  console.log(
    `WORKTREE ${taskId} ${wtPath} branch=${branch} task.md=written ${hookStatus}`,
  );
}
...
function main(): void {
  ...
  const KnownFlags = new Set(["--ready", "--dry-run", "--help", "--create"]);
  for (const a of args) {
    if (a.startsWith("--") && !KnownFlags.has(a)) fail2(`unknown flag: ${a}`);
  }
  ...
  if (args.includes("--create")) {
    ...
    if (existsSync(wtPath)) {
      console.log(
        `WORKTREE ${inferredId} ${wtPath} skipped=(exists) ${hookStatus}`,
      );
      process.exit(0);
    }
    ...
    if (wtAdd.status !== 0)
      fail2(
        `git worktree add failed for ${inferredId}: ${wtAdd.stderr || wtAdd.stdout}`,
      );
    ...
    console.log(
      `WORKTREE ${taskId} ${wtPath} branch=${taskBranch} task.md=written ${hookStatus}`,
    );
    process.exit(0);
  }
  ...
  try {
    preloadedTask = (JSON.parse(vr.out).task ?? undefined) as
      | TaskInfo
      | undefined;
  } catch {
