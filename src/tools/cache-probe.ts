...
async function probe(): Promise<number> {
  ...
  for (let i = 0; i < numRequests; i++) {
    ...
    if (!res.ok) {
      ...
      console.error(
        `Request ${i + 1} failed: HTTP ${res.status} ${text.slice(0, 200)}`,
      );
      if (jsonOutput) {
        console.log(
          JSON.stringify({ error: `HTTP ${res.status}`, request: i + 1 }),
        );
      }
      process.exit(2);
    }
  ...
  if (jsonOutput) {
    ...
    console.log(
      `Verdict: ${verdict} (${cacheHits.length} cache hits on req 2-${numRequests})`,
    );
