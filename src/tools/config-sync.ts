...
function readState(path: string): MaterializeState | null {
  ...
  try {
    ...
    console.error(
      `state file corrupt: ${path}: ${(err as Error).message} (see --help)`,
    );
    process.exit(2);
  }
  const o = raw as Record<string, unknown>;
...
function decide(
  ...
  if (trackedHash === manifestHash && liveHash === manifestHash)
    return { kind: "skip" };
  if (trackedHash !== manifestHash && liveHash === manifestHash)
    return { kind: "copy" };
...
function main(): void {
  ...
  const statePath = statePathFlag
    ...
    : resolve(
        join(HOME, ".local", "state", "weavelog", "config-materialize.json"),
      );
  ...
  for (const [liveRelRaw, trackedRelRaw] of Object.entries(manifest.files)) {
    ...
    entries.push({
      ...
      excluded:
        excludedBy(liveRel, manifest.exclusions) ??
        excludedBy(trackedRel, manifest.exclusions),
    });
  }
  ...
  if (state === null) {
    ...
    for (const e of entries) {
      ...
      if (existsSync(e.livePath))
        state.files[e.liveRel] = sha256File(e.livePath);
    }
  ...
  for (const e of entries) {
    ...
    if (isSymlink(e.trackedPath) || isSymlink(e.livePath)) {
      const offender = isSymlink(e.livePath)
        ? `live ${e.livePath}`
        : `tracked ${e.trackedPath}`;
      console.error(
        `symlink refusal: ${offender} is a symlink — refusing to read or write through links (see --help)`,
      );
      process.exit(2);
    }
    const trackedHash = existsSync(e.trackedPath)
      ? sha256File(e.trackedPath)
      : null;
    ...
    switch (d.kind) {
      ...
      case "conflict":
        refusals.push({
          file: e.liveRel,
          reason:
            "both tracked and live changed since the last materialization",
      ...
      case "missing-tracked":
        missingTracked.push(
          `${e.liveRel} (tracked ${e.trackedRel} at ${e.trackedPath})`,
        );
        outcomes.push({
          liveRel: e.liveRel,
          trackedRel: e.trackedRel,
