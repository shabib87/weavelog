...
const BACKLOG =
  process.env.TASK_FLOW_BACKLOG ?? join(homedir(), ".bun", "bin", "backlog");
...
function main() {
  ...
  if (cmd === "close") {
    ...
    if (editArgs.length === 3)
      fail2("close requires at least one of --note, --check-ac, --status");
    const w = backlog(editArgs);
    if (!w.ok) fail2(`backlog edit failed: ${w.err || w.out}`);
    if (status || acChecks.length) {
      ...
      if (status && t?.status !== status) {
        console.log(
          `CONFLICT ${id}: status=${t?.status ?? "?"} expected=${status}`,
        );
        process.exit(1);
      }
      if (acChecks.length && t?.acceptanceCriteria) {
        ...
        if (unchecked.length) {
          console.log(
            `CONFLICT ${id}: AC not checked after edit: ${unchecked.join(",")}`,
          );
          process.exit(1);
        }
      }
