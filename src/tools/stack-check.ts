...
function readManifest(): Manifest {
  if (!existsSync(manifestPath)) {
    console.error(
      `manifest missing: ${manifestPath} — run the runbook bootstrap first`,
    );
    process.exit(2);
  }
...
export function checkConfigDrift(opts: ConfigDriftOptions): {
  ...
  const files: Record<
    string,
    { tracked: string | null; live: string | null; manifest: null }
  > = {};
  if (!existsSync(opts.harnessManifestPath)) {
    return {
      check: { present: false, manifestPath: opts.harnessManifestPath, files },
      drift: [
        `harness manifest missing at ${opts.harnessManifestPath} — ${CONFIG_FIXES}`,
      ],
    };
  }
  ...
  for (const [liveRel, trackedRel] of Object.entries(manifest.files ?? {})) {
    ...
    if (
      ...
      const offender =
        liveNorm.startsWith("..") || liveNorm === "" || isAbsolute(liveNorm)
          ? liveRel
          : trackedRel;
      drift.push(
    ...
    const excluded =
      excludedBy(liveNorm, exclusions) ?? excludedBy(trackedNorm, exclusions);
    if (excluded) {
      files[liveRel] = { tracked: null, live: null, manifest: null };
    ...
    const trackedHash = existsSync(trackedPath)
      ? sha256File(trackedPath)
      : null;
    ...
    if (trackedHash === null) {
      ...
      drift.push(
        `config drift: ${liveRel} — live file missing at ${livePath}; ${CONFIG_FIXES}`,
      );
...
export function checkDiagramDesign(
  ...
  if (!current) {
    return {
      check,
      drift: [
        `diagram-design git metadata unavailable at ${repoDir} (pinned ${pinned})`,
      ],
    };
  }
...
export function checkProxyMode(
  stats: unknown,
  expectedMode: string,
  baseUrl: string,
): CheckResult {
...
export function checkPiModelsConfig(
  models: unknown,
...
export function checkThresholds(
  ...
  const ctx =
    typeof s?.minContextTokens === "number" ? s.minContextTokens : undefined;
  const msg =
    typeof s?.minMessageChars === "number" ? s.minMessageChars : undefined;
  const check: Record<string, unknown> = {
...
export async function runProxyChecks(
  ...
  if (statsData !== UNSET) {
    if (statsData === null) {
      ...
      drift.push(
        `invalid structure — expected an object at ${opts.proxyBaseUrl}/stats (got null)`,
      );
    } else {
  ...
  if (!existsSync(opts.piModelsPath)) {
    ...
    if (parsed !== UNSET) {
      if (parsed === null) {
        ...
        drift.push(
          `invalid structure — expected an object at ${opts.piModelsPath} (got null)`,
        );
      } else {
  ...
  if (!existsSync(opts.settingsPath)) {
    ...
    if (parsed !== UNSET) {
      if (parsed === null) {
        ...
        drift.push(
          `invalid structure — expected an object at ${opts.settingsPath} (got null)`,
        );
      } else {
        const t = checkThresholds(
          parsed,
          opts.minContextTokens,
          opts.minMessageChars,
        );
...
async function proxyMain(reportPath: string, notify: boolean) {
  const proxyBaseUrl =
    process.env.STACK_CHECK_PROXY_BASE ?? "http://127.0.0.1:8788";
  const piDir = process.env.STACK_CHECK_PI_DIR ?? join(HOME, ".pi", "agent");
  const opencodeConfigPath =
    process.env.STACK_CHECK_OPENCODE_CONFIG ??
    join(HOME, ".config", "opencode", "opencode.jsonc");
  const extDir =
    process.env.STACK_CHECK_HEADROOM_EXT ??
    join(piDir, "extensions", "headroom");
  const settingsPath =
    process.env.STACK_CHECK_HEADROOM_SETTINGS ??
    join(piDir, "headroom", "settings.json");
  const piModelsPath = join(piDir, "models.json");
  const expectedMode = "cache";
  const piExpectedBaseUrl =
    process.env.STACK_CHECK_PI_BASE_URL ?? `${proxyBaseUrl}/v1`;
...
async function main(reportPath: string, notify: boolean) {
  ...
  // 1. headroom
  const currentHeadroom = run(`${HOME}/.local/bin/headroom`, [
    "--version",
  ]).replace(/^headroom, version /, "");
  const updateCheck = run(`${HOME}/.local/bin/headroom`, ["update", "--check"]);
  const latestHeadroom =
    (updateCheck.match(/(\d+\.\d+\.\d+)/g) || []).pop() ?? currentHeadroom;
  ...
  const backlogBin =
    process.env.STACK_CHECK_BACKLOG_BIN ?? `${HOME}/.bun/bin/backlog`;
  if (!existsSync(backlogBin)) {
    ...
    drift.push(
      `backlog.md binary missing at ${backlogBin} (manifest ${manifest.backlogMd})`,
    );
  } else {
    ...
    if (currentBacklog !== manifest.backlogMd) {
      drift.push(
        `backlog.md binary ${currentBacklog} != manifest ${manifest.backlogMd}`,
      );
    } else if (
      latestBacklog !== "(unknown)" &&
      latestBacklog !== manifest.backlogMd
    ) {
  ...
  const skillsDir =
    process.env.STACK_CHECK_SKILLS_DIR ?? join(HOME, ".agents", "skills");
  const dd = checkDiagramDesign(
    manifest.diagramDesignCommit,
    join(skillsDir, "diagram-design"),
  );
  ...
  const models = (await fetchJson(
    process.env.STACK_CHECK_OPENROUTER_MODELS_URL ??
  ...
  const pricingBin =
    ...
    toolScript("sync-model-pricing").path;
  ...
  const configLiveDir =
    process.env.STACK_CHECK_CONFIG_LIVE_DIR ??
  ...
  if (existsSync(liveAgentsMd)) {
    const ptr = checkPointerTargets(
      readFileSync(liveAgentsMd, "utf8"),
      liveAgentsMd,
      HOME,
    );
    ...
    drift.push(
      `config drift: live global AGENTS.md missing at ${liveAgentsMd}; ${CONFIG_FIXES}`,
    );
  }

  // 9. markitdown version drift (mirrors the headroom/backlogMd binary checks)
  const mi = checkMarkitdown(
...
if (
  ...
  const reportPath =
    jsonFlag >= 0 ? args[jsonFlag + 1] : join(reportsDir, defaultName);
