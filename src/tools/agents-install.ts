...
const HOME = homedir();
const manifestPath =
  flagValue("--manifest") ?? join(HOME, ".agents", "stack-versions.json");
const statePath =
  flagValue("--state") ??
...
const lockPath = /\.json$/.test(statePath)
  ...
  : `${statePath}.lock`;
const pythonPath =
  flagValue("--python") ?? join("/opt/homebrew/bin/python3.13");

// --- phase 4 (opencode-config) ----------------------------------------------

const configSyncPath =
  process.env.AGENTS_INSTALL_CONFIG_SYNC ?? toolScript("config-sync").path;
...
if (applyPhase === "4") {
  ...
  // .ts targets need tsx (a .ts tool may import siblings); compiled .js runs plain.
  const tsxPrefix = configSyncPath.endsWith(".ts")
    ? ["--import", String(import.meta.resolve("tsx"))]
    : [];
  const r = spawnSync(
    process.execPath,
    [
  ...
  if (r.error) {
    console.error(
      `phase 4 apply failed: could not spawn config-sync: ${r.error.message}`,
    );
    process.exit(1);
  }
...
function validateManifest(m: unknown): string[] {
  ...
  if (
    ...
    !/^\d{4}-\d{2}-\d{2}$/.test(o.updatedAt)
  ) {
    errors.push(
      `updatedAt must be a YYYY-MM-DD date (got ${JSON.stringify(o.updatedAt)})`,
    );
  }
  if (
...
function validateState(s: unknown): string[] {
  ...
  for (const [id, entry] of Object.entries(
    o.phases as Record<string, unknown>,
  )) {
    if (!PHASE_IDS.has(id)) {
    ...
    if (
      (e.status === "complete" || e.status === "skipped") &&
    ...
    if (
      ...
      (!Array.isArray(e.created) ||
        !e.created.every((x) => typeof x === "string" && x.trim()))
    ) {
      errors.push(
        `phase ${id}: created must be an array of non-empty path strings`,
      );
    }
    if ("backups" in e) {
      const ok =
    ...
    if (
      ...
      errors.push(
        `phase ${id}: provenance must be an object (non-secret metadata only)`,
      );
    }
  }
  return errors;
...
function main() {
  ...
  try {
    ...
    console.error(
      `manifest unparseable: ${manifestPath}: ${(err as Error).message}`,
    );
    process.exit(2);
  }
  ...
  if (stateExists) {
    ...
    try {
      ...
      stateErrors.push(
        `state file corrupt: ${statePath}: ${(err as Error).message}`,
      );
    }
    if (parsed !== undefined) {
      ...
      if (errs.length === 0) {
        statePhases = (
          parsed as { phases: Record<string, { status?: string }> }
        ).phases;
      }
    }
  }
  ...
  const phases = PHASES.map((p) => ({
    ...
    implemented:
      p.id === "4"
        ? existsSync(configSyncPath) &&
          harnessManifestParses(harnessManifestPath)
        : false,
    state: statePhases[p.id]?.status ?? "pending",
    prerequisites: p.prerequisites,
    prereqsOk: p.prerequisites.every(
      (name) => prerequisites[name]?.ok === true,
    ),
  }));
