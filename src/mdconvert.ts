 ...
 *   <home>/.agents/state/mdconvert/converted/<sha1(path+mtimeNs)>-<name>.md
 ...
 *   MDCONVERT_CACHE_DIR cache root (default $HOME/.agents/state/mdconvert/converted)
 */
...
export interface MdconvertOptions {
	...
	binaryPath?: string;
	/** Cache root. Default: $MDCONVERT_CACHE_DIR or $HOME/.agents/state/mdconvert/converted. */
	cacheDir?: string;
...
export async function mdconvert(file: string, opts: MdconvertOptions = {}): Promise<number> {
	...
	const cacheDir =
		...
		join(home, ".agents", "state", "mdconvert", "converted");
	const run: RunCmd =
		opts.run ??
