...
export const GENERAL_LABELS: ReadonlySet<string> = new Set(["harness", "dogfood", "deferred"]);
...
function matchesHarnessDevRepo(name: string): boolean {
	...
	return HARNESS_DEV_REPOS.some(
		(canonical) => n === canonical || n === `${canonical}-dev` || n === `${canonical}.dev`,
	);
}
...
export function isHarnessDevContext(input: HarnessDevInput = {}): boolean {
	...
	if (urls.some((url) => matchesHarnessDevRepo(repoNameFromUrl(url)))) return true;
...
export function decideGeneralLabel(input: LabelDecisionInput): GeneralLabelDecision {
...
export function decideMilestone(input: MilestoneDecisionInput): MilestoneDecision {
	...
	if (hasVersionScope) {
		...
		return {
			...
			reason: "version scope (v1/v2) — migration-time-only signal assigning the version milestone",
		};
	}
	...
	return {
		milestone: null,
		reason: "no version scope and no wayfinder map — none (no milestone assigned)",
	};
}
 ...
 */
export function scaffoldVocabulary(): { reserved: readonly string[]; general: readonly string[] } {
	return {
...
export function hasLabel(labels: string[] | null | undefined, label: string): boolean {
	return (labels ?? []).some((l) => normalizeLabel(l) === normalizeLabel(label));
}
 ...
 * 'High' both pass. */
export const CONFIGURED_PRIORITIES: ReadonlySet<string> = new Set(["high", "medium", "low"]);
...
export function isPlaceholderDescription(description: string): boolean {
	...
	return d === DESCRIPTION_PLACEHOLDER || d.startsWith(CREATE_MODE_PLACEHOLDER_PREFIX);
}
...
export function isSpecApproved(labels: string[] | null | undefined): boolean {
	return (labels ?? []).some((label) => String(label).toLowerCase() === SPEC_APPROVED_LABEL);
}
 ...
 * SATISFIED). */
export function claimGate(task: ParsedTask, options: ClaimGateOptions = {}): GateResult {
	const errors: string[] = [];
	const warnings: string[] = [];

	const description = (task.description ?? "").trim();
	if (!description) {
		errors.push('description is empty — add one with `backlog task edit <id> -d "what & why"`');
	...
	const housekeeping = HOUSEKEEPING_TYPES.has(String(task.type ?? "").toLowerCase());
	for (const ac of acs) {
...
export function parseTaskCreateArgs(input: string | string[]): CreateArgsParse {
	...
	const collectComma = (out: string[], value: string | undefined) => {
		if (value === undefined || value.startsWith("--") || value.trim() === "") return;
		out.push(
			...value
				.split(",")
				.map((s) => s.trim())
...
export function createGateCheck(
	...
	if (selfId && deps.some((d) => String(d).toLowerCase() === selfId.toLowerCase())) {
	...
	if (options.existingDepMap && selfId) {
		...
		const stack = deps.map((d) => String(d).toLowerCase()).filter((d) => d !== lowerSelf);
		let cycle = false;
...
export const AC_MUTATION_FLAGS = ["--clear-ac", "--remove-ac", "--acceptance-criteria"];

/** AC-gutting gate: true when a spec-approved task's edit carries an
 * AC-mutating flag (--clear-ac / --remove-ac / --acceptance-criteria). */
export function acMutationCheck(args: string[], isSpecApproved: boolean): boolean {
	if (!isSpecApproved) return false;
	return args.some((a) => AC_MUTATION_FLAGS.includes(a) || a.startsWith("--acceptance-criteria="));
}
...
export function preCommitCheck(
	...
	const staged = runner(["diff", "--cached", "--name-only", "--diff-filter=ACM"], { cwd });
	...
	for (const file of files) {
		...
		const oldCount = headContent === null ? 0 : countAcceptanceCriteria(headContent);
