...
async function fetchCatalog(): Promise<Map<string, any>> {
	...
	try {
		...
		console.error(`OpenRouter API unavailable; no writes performed. (${String(e)})`);
		process.exit(4);
	}
}
...
for (const id of models) {
	...
	if (!sane(inPerToken) || !sane(outPerToken)) {
		console.error(`implausible pricing for ${id}: in=${inPerToken} out=${outPerToken} (per token)`);
		process.exit(3);
	}
	...
	if (mode === "check") {
		...
		if (existing.headroom_synced && existing.input_cost_per_token > 0) {
			const drift = Math.abs(existing.input_cost_per_token - inPerToken) / inPerToken;
