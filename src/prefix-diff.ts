...
function parseJsonl(content: string): LogEntry[] {
	...
	for (let i = 0; i < lines.length; i++) {
		try {
			...
			console.error(`Error: malformed JSON at line ${i + 1}: ${lines[i].slice(0, 100)}`);
			process.exit(2);
		}
	}
	return entries;
...
function findFirstDiff(a: unknown[], b: unknown[]): { index: number; detail: string } | null {
...
function determineVerdict(
	...
	const request_diff = findFirstDiff(reqSlice1, reqSlice2);
	const request_stable = request_diff === null;

	let compressed_diff: { index: number; detail: string } | null = null;
	let compressed_stable = true;

	if (comp1.length > 0 || comp2.length > 0) {
		const compSlice1 = comp1.slice(0, prefixLen);
		const compSlice2 = comp2.slice(0, prefixLen);
		compressed_diff = findFirstDiff(compSlice1, compSlice2);
		compressed_stable = compressed_diff === null;
	}

	const contentRewrite =
		hasContentRewriteTransforms(transforms1) || hasContentRewriteTransforms(transforms2);
	...
	if (!request_stable) {
		...
		evidence = `request prefix changed at ${request_diff!.detail}`;
	...
	} else if (!compressed_stable) {
		verdict = "TOKEN_MODE_HISTORY_REWRITE";
		evidence = `request prefix stable but compressed prefix differs at ${compressed_diff!.detail}`;
	} else {
		verdict = "STABLE";
...
if (entries.length < 2) {
	console.error("Error: need at least 2 entries to diff (got " + entries.length + ")");
	process.exit(2);
}
	...
	for (const p of pairs) {
		console.log(
			`  pair[${p.pair[0]}→${p.pair[1]}]: ${p.verdict} — ${p.evidence}` +
				...
				(p.compressed_diff ? ` | compressed_diff: ${p.compressed_diff.detail}` : ""),
		);
	}
}
process.exit(0);
