 ...
 * Activation model (TASK-71, 2026-09-07): hooks are ordinary project modules
 * following the repo standard (single path module: src/tools/tool-paths.ts).
 * They are NOT copied 1:1 into a host. Each outer harness gets its own install
 * adapter that wires them as that harness requires — opencode first, then pi,
 * claude, codex (per-harness adapters = follow-up work; the old
 * "copy this file to ~/.config/opencode/plugins/" model is retired).
 * Rollback: remove the adapter registration — instant.
...
function denylist(home: string): Array<{ pattern: RegExp; message: string }> {
  const HomeAlt = String.raw`(?:~|\$HOME|\$\{HOME\}|${escapeRegExp(home)})`;
  return [
    {
      pattern: new RegExp(
        String.raw`chmod\s+(?:-R\s+)?600\s+${HomeAlt}/\.headroom(?:\s|/|$)`,
      ),
      message:
    ...
    {
      pattern: new RegExp(
        String.raw`chmod\s+(?:-R\s+)?600\s+${HomeAlt}/\.local/pipx(?:\s|/|$)`,
      ),
      message:
        "chmod 600 strips execute bit from the pipx directory — use chmod 700",
    },
    {
      pattern: new RegExp(
        String.raw`rm\s+-rf\s+${HomeAlt}/\.headroom(?:\s|/|$)`,
      ),
    ...
    },
    {
      ...
      pattern: new RegExp(
        String.raw`rm\s+-rf\s+${HomeAlt}/\.local/pipx/venvs/headroom-ai(?:\s|/|$)`,
      ),
...
export function createHooks(deps: EnforceDeps): EnforceHooks {
  ...
  let createQualityIssues: ((command: string) => CreateGateOutcome) | null =
    null;
  const enforceCreateQuality = async (command: string): Promise<void> => {
    if (createQualityIssues === null) {
      try {
        const mod = (await import(toolScriptHref("task-validate"))) as {
        ...
        createQualityIssues = (cmd: string): CreateGateOutcome => {
          ...
          if (!parsed.hasDescription)
            refuse.push("missing description (-d/--description)");
          if (parsed.acCount === 0)
            refuse.push("zero acceptance criteria (--ac)");
          if (parsed.hasNoDodDefaults) refuse.push("--no-dod-defaults");
          const gate = mod.createGateCheck(parsed, {
            isBacklogProject:
              deps.isBacklogProject ??
              existsSync(join(hookCwd, "backlog", "config.yml")),
  ...
  const ResearchDir = join(home, ".agents", "docs", "research");
  const PlansDir = join(home, ".agents", "docs", "plans");
  const machineryScript = (
    base: string,
  ): { path: string; prefix: string[] } => {
    const { path, tsx } = toolScript(base);
    return { path, prefix: tsx ? ["--import", "tsx"] : [] };
  };
  const Fm = machineryScript("frontmatter-check");
  const Reviewer = machineryScript("reviewer-loop");
  const FrontmatterCheck = Fm.path;
  const ReviewerLoop = Reviewer.path;
  const HeadroomBin = join(home, ".local", "bin", "headroom");
  const LearnTarget = join(home, ".agents", "docs", "learned-patterns.md");
  const Denylist = denylist(home);
  const HarnessRoot = `${join(home, ".agents")}/`;
  ...
  function boundedRemember(store: Set<string>, key: string): void {
    if (store.size >= MaxTrackedSessions) {
  ...
  function stashWarning(sessionId: string, warning: string): void {
    if (pendingConfigWarnings.size >= MaxTrackedSessions) {
  ...
  function isLiveHarnessPath(p: string): boolean {
    try {
      return canonicalPath(p).startsWith(LiveHarnessRoot);
    } catch {
      return resolve(p).startsWith(LiveHarnessRoot);
    }
  }
  ...
  return {
    ...
    "tool.execute.after": async (input, output) => {
      ...
      if (filePath.startsWith(`${ResearchDir}/`) && exists(FrontmatterCheck)) {
      ...
      if (filePath.startsWith(`${PlansDir}/`) && exists(ReviewerLoop)) {
        output.metadata = {
          ...output.metadata,
          review_reminder: `Run \`bun ${ReviewerLoop} --plan ${filePath} --models z-ai/glm-5.3-flash,moonshotai/kimi-k3\` for cross-model review (runbook SOP §6.3).`,
        };
      }
    },
    ...
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "edit" || input.tool === "write") {
        ...
        if (!filePath.startsWith(HarnessRoot)) return; // not the harness repo
      ...
      // throws synchronously in opencode 1.18.18 — withTimeout only catches
      // async rejections, so the catch would fire and fail open every time.
      if (COMMIT_RE.test(command) && exists(FrontmatterCheck)) {
        try {
          const staged = runCmd(
            "git",
            ["diff", "--cached", "--name-only", "--", `${ResearchDir}/`],
            { timeout: timeouts.checkMs },
          );
    ...
    event: async ({ event }) => {
      ...
      if (typeof sessionId !== "string" || learnedSessions.has(sessionId))
        return;
      ...
      try {
        const result = await withTimeout(
          (apply
            ? $`${HeadroomBin} learn --agent opencode --apply --target ${LearnTarget} --model ${LEARN_MODEL}`
            : $`${HeadroomBin} learn --agent opencode --target ${LearnTarget} --model ${LEARN_MODEL}`
          )
            .quiet()
            .nothrow(),
          timeouts.learnMs,
