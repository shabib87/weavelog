...
export function resolveModels(modelsArg?: string, excludeArg?: string): string[] {
...
if (
	...
	const rubric = opt("--rubric") ? readFileSync(opt("--rubric") as string, "utf8") : DEFAULT_RUBRIC;
	...
	async function review(model: string): Promise<{ model: string; content: string; usage: any }> {
		...
		return {
			model,
			content: content ?? "(no content - thinking consumed budget; retry with larger --max-tokens)",
			usage: resp.usage,
		};
	}
	...
	for (let i = 0; i < results.length; i++) {
		...
		const costUsd =
			(r.value.usage?.prompt_tokens * inM + r.value.usage?.completion_tokens * outM) / 1e6;
		totalUsd += costUsd;
	...
	if (totalUsd > budgetUsd)
		fail(`BUDGET CAP EXCEEDED ($${totalUsd.toFixed(2)} > $${budgetUsd.toFixed(2)})`, 1);
}
