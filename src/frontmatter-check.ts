...
const ARCH_STATUSES = ["draft", "in-review", "approved", "superseded", "archived"] as const;
...
const KNOWN_FLAGS = new Set(["--help", "--index", "--fix", "--source", "--dry-run", "--schema"]);
...
if (schemaArg !== "research" && schemaArg !== "architecture") {
	console.error(`--schema value must be 'research' or 'architecture', got: ${schemaArg}`);
	process.exit(2);
}
...
if (sourceFlag >= 0 && (sourceFlag + 1 >= args.length || args[sourceFlag + 1].startsWith("--"))) {
	console.error("--source requires a value");
	process.exit(2);
}
const fixSource = sourceFlag >= 0 ? args[sourceFlag + 1] : "(unknown — manual entry required)";
...
const scanDirs =
	dirs.length > 0
		...
		: schema === "architecture"
			...
			: ["research", "plans", "spec"].map((d) => join(HOME, ".agents", "docs", d));
...
function validate(
	...
	if ("date" in fm) {
		...
		if (!/^\d{4}-\d{2}-\d{2}$/.test(d)) v.push(`date is not ISO 8601 (YYYY-MM-DD): ${d}`);
	...
	if ("status" in fm && !STATUSES.includes(fm.status as (typeof STATUSES)[number])) {
		v.push(`status "${fm.status}" is not in the enum: ${STATUSES.join(" | ")}`);
	}
	if ("sources" in fm && (!Array.isArray(fm.sources) || fm.sources.length === 0)) {
		v.push("sources must be a non-empty array");
	}
	if ("models_used_for_research" in fm && !Array.isArray(fm.models_used_for_research)) {
	...
	if ("supersedes" in fm) {
		if (typeof fm.supersedes !== "string") {
			...
			v.push(`supersedes target does not exist in the same directory: ${fm.supersedes}`);
		}
	}
	if ("review_rounds" in fm && !Number.isInteger(fm.review_rounds)) {
		v.push("review_rounds must be an integer");
	}
	if ("total_review_cost_usd" in fm && typeof fm.total_review_cost_usd !== "number") {
		v.push("total_review_cost_usd must be a number");
	}
	if ("reviewer_corrections_applied" in fm && !Array.isArray(fm.reviewer_corrections_applied)) {
		v.push("reviewer_corrections_applied must be an array");
	}
	if (fm.status === "superseded" && !supersededBy.has(join(doc.dir, doc.file))) {
...
function validateArch(doc: DocEntry): { violations: string[]; warnings: string[] } {
	const v: string[] = [];
	const w: string[] = [];
	const fm = doc.fm ?? {};
	...
	if ("status" in fm && !ARCH_STATUSES.includes(fm.status as (typeof ARCH_STATUSES)[number])) {
		v.push(`status "${fm.status}" is not in the architecture enum: ${ARCH_STATUSES.join(" | ")}`);
	}
	if ("type" in fm) {
		...
		if (!Array.isArray(fm.related_to)) {
			...
			for (const entry of fm.related_to) {
				...
				if (!target || !Array.isArray(targetRel)) {
					w.push(`missing reciprocity: ${entry} does not list this file in its related_to`);
					continue;
				}
...
function fixDoc(doc: DocEntry): void {
	...
	const requiredKeys = schema === "architecture" ? ARCH_REQUIRED_KEYS : REQUIRED_KEYS;
	...
	const defaults: Record<string, unknown> =
		schema === "architecture"
			? {
					...
					topic: prefix ? prefix[2].replaceAll("-", " ") : basename(doc.file, ".md"),
					status: "draft",
					type: "architecture",
...
function main() {
	...
	for (const doc of docs) {
		...
		if (typeof s === "string" && s !== "none") supersededBy.set(join(doc.dir, s), doc.file);
	}
	...
	const files = docs.map((doc) => {
		...
		const dirFiles = new Set(docs.filter((d) => d.dir === doc.dir).map((d) => d.file));
