...
- [ ] #6 WHEN the run completes THEN evidence (sw_vers, uname -m, node -v, doctor output + exit code, ledger tail) SHALL be recorded in TASK-45 notes sanitized of absolute home paths, after the exact teardown: evidence out -> exit stranger shell -> pkill -u stranger -> deleteUser -> revert home mode -> remove /Users/Shared staging -> post-asserts green
- [ ] #7 WHEN a tag or publish is attempted THEN this ticket SHALL be closed alongside TASK-53, TASK-54, TASK-56, TASK-62, TASK-63, TASK-64 and TASK-66 before the publish-gate marker flips
- [ ] #8 WHEN the ticket is picked up THEN it SHALL run only AFTER TASK-53 and TASK-54 closure and the publish-gate marker flip per TASK-45 Step 4 ordering
<!-- AC:END -->
