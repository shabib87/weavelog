...
docs/architecture/ currently has only ONE diagram: diagrams/loop-factory.html (TASK-15, via diagram-design skill). The remaining architecture docs are text-only. This task adds FOUR diagrams (config-sync-flow is DEFERRED to a follow-up gated on TASK-23 Done — it depicts a mechanism that doesn't exist yet):

| Diagram | Home doc (must link it) | Scope |
|---|---|---|
| tool-boundaries.html | docs/architecture/tool-boundaries.md | tool loop-POSITION placement on the phase x loop model (not a re-render of the table) |
| backlog-lifecycle.html | docs/architecture/backlog-lifecycle.md | task STATE MACHINE: create -> plan -> worktree -> verify -> merge -> Done, with backlog.md field mapping (distinct from loop-factory which is phases/roles/boundary) |
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 WHEN ls docs/architecture/diagrams/ is run THEN it lists loop-factory.html, tool-boundaries.html, backlog-lifecycle.html, conductor-dispatch.html, subagent-roster.html, and README.md (config-sync-flow is DEFERRED — gated on TASK-23 Done, not part of this task)
...
- [x] #5 WHEN docs/architecture/diagrams/README.md is read THEN it lists each diagram, a one-line description, its source-doc link, and the generated date
<!-- AC:END -->
