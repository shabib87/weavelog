...
4. Correction of the stale 'display-only / no export API' claim in the difit bullet of docs/trd/worktree-discipline.md ONLY (pre-sync wording stays owned by TASK-50 to avoid a doc-edit collision).
...
- [ ] #7 WHEN evidence contradicts docs/trd/worktree-discipline.md THEN the stale display-only / no-export-API claim in the difit bullet is corrected in this task so the single-source doc matches verified difit 5.x behavior, scoped to that claim only (pre-sync wording remains TASK-50-owned)
