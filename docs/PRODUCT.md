...
weavelog is a **deterministic CLI that composes an opinionated,
...
| Command | Job |
|---|---|
| `weavelog init` | install opinionated deps, materialize `~/.agents/*` + `~/.config/opencode/*` (two-step user-modification flow), never silently overwrite managed files |
| `weavelog sync` | dev path: repo → live (dogfood loop) |
| `weavelog update` | release-driven dep bump + re-materialize |
| `weavelog check` | deterministic gates: tests, lint, typecheck, semgrep, secrets, frontmatter, manifest completeness |
| `weavelog doctor` | installed? authenticated? config parses? proxy healthy? versions pinned? arm64? |
| `weavelog scaffold --project` | project scaffold: backlog init, AGENTS.md, docs/research, ADRs, .gitignore, .env.example |
...
weavelog is **opinionated, not generic.** It encodes one specific philosophy
...
2026 onward); weavelog evolves with the series, with attribution (see
...
- **Host-extensible.** Core weavelog composes hosts one at a time across
...
|---|---|---|---|
| **1. Fixed (constitution)** | Engineering philosophy (YAGNI, SOLID, KISS, DRY, TDD, maker/checker, security), loop shape (spec, implement, verify, document), open standards (Agent Skills, AGENTS.md), host-composed pattern, evidence-over-claims | weavelog core | No. If user disagrees, weavelog is the wrong tool. |
| **2. Opinionated defaults** | Specific model roster, budget default, agent role prompts, CI pipeline shape, hook configs, which host first | weavelog ships defaults | Yes. `weavelog init` asks and writes them; flags override. User edits after. weavelog never overwrites user modifications. |
...
**Principle:** weavelog is opinionated about methodology (Layer 1),
...
weavelog owns the defaults. Layer 1 is fixed. If a user wants to change
...
- **Does NOT run the loop.** The composed host (opencode first, then pi,
  Claude Code, Codex) runs the loop. weavelog sets up, gates, and audits.
...
- **Does NOT author platform skills.** Subject-matter experts do. weavelog
...
- **Does NOT reinvent engineering methodology.** superpowers does (TDD, code
  review, verification, planning, debugging). weavelog composes it, with
  attribution.
- **Does NOT ship a hosted/SaaS product in v1.** weavelog is a local CLI.
...
**v1: the author.** weavelog is built for the author's own workflows
...
fine - the scaffold is the wedge, not the moat. It gets weavelog installed
...
**"I built this for me and it worked so well I open-sourced it."** weavelog
is validated by the author using it on real projects, with the data published.
If the author stops using weavelog, PMF is falsified - there is no fallback
audience.
...
|---|---|---|
| weavelog itself | Devex tool (dogfooding) | The tool builds itself - the system loop made literal |
...
v1.0 proves on the weavelog + blog projects. The mobile proof project
...
- The process (how weavelog was used, step by step)
...
weavelog is open-source for **credibility and influence, not revenue.**

- **Personal leverage:** weavelog is the author's tool first; OSS is so
  others can self-host and the author gets reputation/portfolio value.
- **Influence:** the author wants weavelog's ideas (evidence-grade agentic
...
**The author decides where weavelog goes.** This is a product principle,
not just a license choice. It informs:

- License: Apache-2.0 + NOTICE. The weavelog name/logo © the author -
...
- Plugin authority: official weavelog plugins are authored/curated by the
  author. Community plugins conform to the Agent Skills standard. weavelog
  validates but does not endorse community plugins.

## Quality bar

**weavelog is a real engineering product for real engineers.** This is not a
...
production devex tool. If weavelog does not meet this bar, it does not ship.
...
harnesses adjacent to or overlapping weavelog's niche:

| Harness | What it is | Relationship to weavelog |
|---|---|---|
...
| **outrigger** (`dwijenpatel/outrigger`) | evidence discipline: held-out examiner, null arms, harness-vs-null measurement, run ledger | **Evidence discipline reference.** Its own 5.9x experiment demoted heavier gates - weavelog's "process hygiene is not moat" lesson. |
| **tiller-ai** (`hmSchuller/tiller-ai`) | `npx init` scaffolder for Claude Code/Copilot CLI/OpenCode; managed-files manifest + upgrade | **Distribution reference.** The managed-files manifest pattern weavelog's manifest draws from. Dormant since 2026-03. |
...
**Where weavelog differs:** none of the scanned harnesses combine model
...
distribution but is dormant. weavelog's bet: the evidence layer (Run
...
- **NORTH_STAR** references this for the "what is weavelog" framing.
