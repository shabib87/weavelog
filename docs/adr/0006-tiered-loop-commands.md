...
- **Before the weave:** thinking work, done with you, never solo. The PRD (product
  ...
  evidence; you steer; you decide. The task also does not enter the backlog on its
  own: the claim-time spec check (when the task enters work; TASK-51, `spec-approved`)
  is a human checkpoint that approves the description and acceptance criteria before
  any build planning starts.
...
2. The agent autonomously drafts the step-by-step implementation plan.
3. **Human gate 1 — plan approval.** You read the plan: approve, kickback, or reject.
